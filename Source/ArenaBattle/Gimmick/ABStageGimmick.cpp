...
AABStageGimmick::AABStageGimmick()
{
	...
	StageTrigger->SetRelativeLocation(FVector(0.f, 0.f, 0.f));
	...
	for (FName GateSocket : GateSockets)
	{
		...
		GateTrigger->SetBoxExtent(FVector(100.f, 100.f, 100.f));
...
// 해당 함수는 모든 속성이 변경될때마다 호출이 됩니다.
// 에디터에서 속성값을 바꾸면 해당 함수가 호출되는 언리얼 엔진에서 제공해주는 함수입니다.
...
void AABStageGimmick::SpawnRewardBoxes()
{
	...
	{
		FTransform WorldSpawnTransform = FTransform(GetActorLocation() + RewardBoxLocation.Value + FVector(0.f, 0.f, 30.f));
